...
def get_args_parser():
    ...
    parser.add_argument(
        "--inverse_lr", action="store_true", default=False, help="Use inverse lr scheduler"
    )
    parser.add_argument(
        "--no_lr_scale", action="store_true", default=False, help="Do not scale lr by mask_ratio"
    )
    ...
    parser.add_argument(
        "--use_fm", nargs="+", type=int, default=[-1], help="Feature maps to use for decoder"
    )
    parser.add_argument(
        "--use_input", action="store_true", default=False, help="use input as a feature map"
    )
    parser.add_argument(
        "--self_attn", action="store_true", default=False, help="use self attention in decoder"
    )

    parser.add_argument(
        "--enable_flash_attention2", action="store_true", default=False, help="Use flash attntion 2"
    )
...
def main(args):
    ...
    dataloader_cls = (
        MultiEpochsDataLoader if args.multi_epochs_dataloader else torch.utils.data.DataLoader
    )
